...
def main():
    ...
    for path_image, path_mask in zip(paths_image, paths_mask):
        ...
        for idx, bbox in enumerate(bboxes):
            ...
            inpainted_512 = inference(image_512, mask_512)

            inpainted_512_large = cv2.resize(inpainted_512, (crop_size, crop_size), interpolation=cv2.INTER_LINEAR)            
            mask_large = np.expand_dims(mask_large, axis=2)
            mask_large = mask_large.astype(np.float32) / 255
            output_large = inpainted_512_large * mask_large + image_large * (1. - mask_large)
            filename = args.tmp_dir + '/' + os.path.splitext(os.path.basename(path_image))[0] + f'_inpainted-{idx}.jpg'
